...
@Slf4j
@Service
public class CallbackHandler extends
        BaseHandler<CallbackCommand, TransactionModel, TransTransactionContext<CallbackCommand, TransactionModel>> {
    ...
    public TransTransactionContext<CallbackCommand, TransactionModel> initContext(final CallbackCommand request) {
        // 1. 初始化交易上下文
        TransTransactionContext<CallbackCommand, TransactionModel> context =
    ...
    private void assemblePayOrderBeforeUpdate(final CallbackCommand command, final TransPayOrderEntity payOrder) {
        ...
        AssertUtils.assertNotNull(orderStatusEnum, INVALID_PAY_ORDER_STATUS_ERROR,
                "invalid payOrder status：" + payOrderStatus);
