...
- **健康检查工具类**：标准化的健康检查数据结构表示，包括组件健康状态值对象、健康状态响应值对象和健康状态枚举

同时提供真实测试用例路径与性能优化建议，帮助读者在生产环境中安全高效地使用这些工具。

## 项目结构
高级工具类主要位于common-util模块的util包内，并配套apm子包用于监控追踪；健康检查相关的值对象位于health子包中；测试用例位于common-util的test目录中。
...
subgraph "健康检查值对象"
CHVO["ComponentHealthVO.java"]
HHSV["HealthStatusVO.java"]
HSENUM["HealthStatusEnum.java"]
end
APM --> AM
APM --> MSG
APM --> TX
...
- **健康检查工具类**：提供标准化的健康检查数据结构，包括组件健康状态值对象、健康状态响应值对象和健康状态枚举，支持UP/DOWN/WARN三种状态
...
- [common-util/src/main/java/com/magicliang/transaction/sys/common/util/health/ComponentHealthVO.java:19-30](file://common-util/src/main/java/com/magicliang/transaction/sys/common/util/health/ComponentHealthVO.java#L19-L30)
- [common-util/src/main/java/com/magicliang/transaction/sys/common/util/health/HealthStatusVO.java:19-35](file://common-util/src/main/java/com/magicliang/transaction/sys/common/util/health/HealthStatusVO.java#L19-L35)
- [common-util/src/main/java/com/magicliang/transaction/sys/common/enums/HealthStatusEnum.java:9-40](file://common-util/src/main/java/com/magicliang/transaction/sys/common/enums/HealthStatusEnum.java#L9-L40)

## 架构总览
APM监控工具链采用"监控器-消息-事务"三层架构，结合内部抽象类实现复合模式与线程本地上下文，形成可嵌套、可排序、可自动清理的调用树。健康检查工具类提供标准化的数据结构表示，支持组件级别的健康状态检查与聚合。
...
-status String
-timestamp String
-components Map~String,ComponentHealthVO~
}
...
### 健康检查工具类
- **ComponentHealthVO**：组件健康状态值对象，包含组件状态和详细信息（如响应时间、错误信息等）
- **HealthStatusVO**：健康状态响应值对象，包含整体健康状态、检查时间戳和各组件健康状态详情
- **HealthStatusEnum**：健康状态枚举，提供UP、DOWN、WARN三种状态常量
- **使用场景**：
  - 统一健康检查数据结构表示
  - 支持组件级别的健康状态聚合
  - 提供标准化的健康检查API响应格式
...
+check() ComponentHealthVO
}
class ICustomHealthService {
<<interface>>
+getHealthStatus(component) HealthStatusVO
}
...
+getHealthStatus(component) ApiResult~HealthStatusVO~
}
ComponentHealthVO <|-- HealthStatusVO : "components"
HealthStatusEnum --> HealthStatusVO : "status"
HealthIndicator --> CustomHealthServiceImpl : "聚合检查"
...
- [core-service/src/main/java/com/magicliang/transaction/sys/core/service/ICustomHealthService.java:11-21](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/ICustomHealthService.java#L11-L21)
- [core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/CustomHealthServiceImpl.java:27-82](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/CustomHealthServiceImpl.java#L27-L82)
- [biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java:26-67](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L26-L67)
...
- **健康检查模块**：健康检查工具类与核心服务层的HealthIndicator接口解耦，支持组件化健康检查
...
JDKH --> PC["ProxyConfig"]
CHVO["ComponentHealthVO"] --> HHSV["HealthStatusVO"]
HSENUM["HealthStatusEnum"] --> HHSV
HI["HealthIndicator"] --> CHVO
CHVO --> HHSV
```
...
- 动态代理：
  ...
  - 超时阈值应结合业务场景调整，避免误报
- **健康检查**：
  - 组件健康检查应异步执行，避免阻塞主业务流程
  - 支持组件级别的选择性检查，减少不必要的检查开销
  - 时间戳格式化使用ISO 8601标准，确保跨系统兼容性
...
- **健康检查**：
  - 响应格式异常：检查HealthStatusVO的时间戳格式是否符合ISO 8601标准
  - 组件状态聚合错误：确认HealthStatusEnum的优先级顺序（DOWN > WARN > UP）
  - API参数校验失败：检查HealthController对组件名称的支持列表
...
- [core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/CustomHealthServiceImpl.java:56-71](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/CustomHealthServiceImpl.java#L56-L71)
- [biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java:48-52](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L48-L52)

## 结论
本指南系统梳理了领域驱动交易系统中的高级工具类，覆盖APM监控、HTTP请求、数据库断言、金额计算、ID生成、动态代理以及**健康检查**等关键能力。通过测试用例与架构图解，读者可快速理解并安全地在生产环境中应用这些工具，提升系统的可观测性、稳定性与可维护性。新增的健康检查工具类提供了标准化的数据结构表示，支持组件级别的健康状态检查与聚合，为系统的健康监控提供了坚实的基础。

## 附录
- 测试用例参考：
  - APM事务测试：涵盖单事务、嵌套事务、复杂嵌套、循环嵌套、多线程与线程池场景
  - 事务时间合法性校验：验证起止时间与持续时间的约束关系
  - **健康检查测试**：涵盖组件健康状态聚合、异常处理、时间戳格式验证等场景
...
- [core-service/src/test/java/com/magicliang/transaction/sys/core/service/impl/CustomHealthServiceImplTest.java:48-198](file://core-service/src/test/java/com/magicliang/transaction/sys/core/service/impl/CustomHealthServiceImplTest.java#L48-L198)
- [core-service/src/test/java/com/magicliang/transaction/sys/core/service/impl/health/ApplicationHealthIndicatorTest.java:1-68](file://core-service/src/test/java/com/magicliang/transaction/sys/core/service/impl/health/ApplicationHealthIndicatorTest.java#L1-L68)