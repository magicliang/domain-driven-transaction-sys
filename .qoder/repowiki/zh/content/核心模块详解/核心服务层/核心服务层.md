...
## 更新摘要
**变更内容**
- 新增数据库表计数查询服务组件：IDatabaseCountService接口和DatabaseCountServiceImpl实现
- 添加安全防护机制：表名正则校验、白名单缓存、查询超时控制
- 完整的分层架构：DAL层、Core Service层、Biz Service层的职责分离
- Web控制器：提供RESTful API接口 `/api/v1/database/count`
- 单元测试覆盖：安全校验、白名单缓存、异常处理等场景
...
- 服务接口设计：IPayOrderService、IDatabaseCountService、IDistributedLock等接口的职责与实现
- 分布式锁管理机制：DistributedLockImpl的实现与分布式事务处理要点
- 数据库表计数查询服务：安全防护的表计数查询API设计与实现
- 业务流程编排：从受理到支付再到通知的完整链路与错误处理

## 项目结构
核心服务层位于 core-service 模块，围绕"活动 + 策略 + 服务"的领域驱动设计，将业务流程拆解为可组合、可扩展的活动单元，并通过策略点在运行期选择具体执行路径。新增的数据库表计数服务提供了安全的表计数查询能力。
...
D["service 接口与实现<br/>IPayOrderService / IDatabaseCountService / IDistributedLock / 实现类"]
E["database count service<br/>表计数查询服务"]
end
A --> B
B --> C
D --> A
...
- [IDatabaseCountService.java:9-18](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/IDatabaseCountService.java#L9-L18)
...
- 服务接口与实现：IPayOrderService（订单与请求的填充、更新）、IDatabaseCountService（表计数查询）、IDistributedLock（分布式锁能力）
- 具体策略实现：同步受理 SyncAcceptanceStrategy、RPC 通知 RpcNotificationStrategy
- 数据库表计数服务：DatabaseCountServiceImpl（安全表计数查询实现）
...
## 架构总览
核心服务层采用"活动 + 策略 + 服务"的分层设计：
- 活动层负责业务流程编排与幂等控制
- 策略层负责具体执行分支（受理同步、支付策略、通知策略）
- 服务层提供跨活动的数据与事务能力（订单填充、更新、分布式锁、表计数查询）
- 数据库表计数服务提供安全的表计数查询能力
...
DatabaseCountServiceImpl ..|> IDatabaseCountService
```
...
- [DatabaseCountServiceImpl.java:23-129](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/DatabaseCountServiceImpl.java#L23-129)
...
### 服务接口设计：IPayOrderService、IDatabaseCountService与IDistributedLock
- IPayOrderService：提供订单填充（轻量/完整）、请求填充（支付/通知）、计数与批量查询、更新与事务性写入等能力
- IDatabaseCountService：提供安全的表计数查询能力，支持表名正则校验、白名单缓存、查询超时控制
...
-validateTableNameInWhitelist()
-executeCountWithTimeout()
}
class DistributedLockImpl {
-参数校验
-生命周期钩子
...
### 安全防护机制
数据库表计数服务实现了多层次的安全防护机制，确保查询操作的安全性和稳定性：

- **表名正则校验**：使用正则表达式 `^[a-zA-Z_][a-zA-Z0-9_]*$` 确保表名只包含字母、数字、下划线，且必须以字母或下划线开头
- **白名单缓存**：通过 `ConcurrentHashMap.newKeySet()` 实现线程安全的表名白名单缓存，支持懒加载机制
- **查询超时控制**：设置5秒查询超时时间，防止大数据量表的COUNT查询阻塞
- **双重校验机制**：正则校验作为第一道防线，白名单作为第二道防线，双重保护防止SQL注入

### 核心实现逻辑
DatabaseCountServiceImpl的实现遵循"先校验、后查询"的原则：

1. **表名格式校验**：验证表名格式是否符合正则表达式要求
2. **白名单加载**：首次查询时从 `information_schema.tables` 加载所有用户表名到白名单缓存
3. **白名单校验**：确保查询的表名存在于白名单中
4. **安全查询**：通过 `DatabaseCountSqlProvider` 动态构建SQL语句执行COUNT查询
5. **性能监控**：记录查询耗时，超过阈值时输出警告日志

### 分层架构设计
数据库表计数服务采用了清晰的分层架构：

- **DAL层**：`DatabaseCountMapper` + `DatabaseCountSqlProvider` + `TableMetaMapper`
- **Core Service层**：`IDatabaseCountService` + `DatabaseCountServiceImpl`
- **Biz Service层**：`DatabaseCountController` + `DatabaseCountVO`

### Web API接口
提供RESTful API接口 `/api/v1/database/count`，支持GET请求查询指定表的总记录数。

**章节来源**
- [DatabaseCountServiceImpl.java:23-129](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/DatabaseCountServiceImpl.java#L23-L129)
- [DatabaseCountController.java:19-49](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/DatabaseCountController.java#L19-L49)
- [DatabaseCountMapper.java:12-22](file://common-dal/src/main/java/com/magicliang/transaction/sys/common/dal/mybatis/mapper/DatabaseCountMapper.java#L12-L22)
- [TableMetaMapper.java:14-23](file://common-dal/src/main/java/com/magicliang/transaction/sys/common/dal/mybatis/mapper/TableMetaMapper.java#L14-L23)
- [DatabaseCountSqlProvider.java:11-26](file://common-dal/src/main/java/com/magicliang/transaction/sys/common/dal/mybatis/mapper/DatabaseCountSqlProvider.java#L11-L26)
- [DatabaseCountVO.java:13-33](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/model/vo/DatabaseCountVO.java#L13-L33)

## 依赖分析
- 活动与策略：活动持有策略集合，策略依赖服务接口进行数据写入
- 服务层：IPayOrderService、IDatabaseCountService的实现依赖管理器与转换器，提供事务性更新和安全查询
- 分布式锁：IDistributedLock的实现依赖分布式锁管理器，提供多种加锁语义
- 数据库访问：DatabaseCountServiceImpl依赖DatabaseCountMapper和TableMetaMapper进行数据查询
...
DatabaseCountServiceImpl --> TableMetaMapper
...
- [DatabaseCountController.java:24-25](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/DatabaseCountController.java#L24-L25)
...
- 批量查询与计数：IPayOrderService提供批量未支付/未通知请求查询，减少数据库往返
- 事务边界：策略与服务层在事务内写入，降低并发冲突概率
- 锁粒度：分布式锁用于关键资源互斥，建议按业务域细分锁名，避免热点竞争
- **表计数查询优化**：白名单缓存避免重复查询元数据，查询超时控制防止长时间阻塞

## 故障排查指南
- 活动断言失败：受理/支付/通知的 postExecution 中断言响应字段，若失败需检查上游策略执行与上下文装配
- 策略不生效：确认 decideStrategy 返回的策略点与策略 identify() 匹配
- 分布式锁异常：DistributedLockImpl将业务异常包装为分布式锁异常，关注锁名与过期时间参数
- 通知失败：RpcNotificationStrategy记录异常与响应，检查下游回调地址与请求参数
- **表计数查询失败**：检查表名格式是否符合正则表达式，确认表名是否在白名单中，关注查询超时日志
...
- [DatabaseCountServiceImpl.java:76-81](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/DatabaseCountServiceImpl.java#L76-L81)
- [DatabaseCountServiceImpl.java:99-104](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/DatabaseCountServiceImpl.java#L99-L104)

## 结论
核心服务层通过"活动 + 策略 + 服务"的架构，实现了高内聚、低耦合的业务编排与扩展能力。活动层统一生命周期与幂等控制，策略层承载具体执行分支，服务层提供数据与事务保障，分布式锁确保并发安全。新增的数据库表计数服务进一步丰富了核心服务能力，通过多重安全防护机制确保查询操作的安全性和稳定性。该设计既便于新增策略与活动，又能在复杂业务场景下保持清晰的执行路径与错误处理机制。
...
- 代码示例路径（不含具体代码内容）：
  ...
  - RPC通知策略执行：[RpcNotificationStrategy.java:80-121](file://core-service/src/main/java/com/magicliang/transaction/sys/core/domain/strategy/notification/RpcNotificationStrategy.java#L80-L121)
  ...
  - **表计数查询实现**：[DatabaseCountServiceImpl.java:60-71](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/impl/DatabaseCountServiceImpl.java#L60-L71)
  - **Web API接口**：[DatabaseCountController.java:33-48](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/DatabaseCountController.java#L33-L48)