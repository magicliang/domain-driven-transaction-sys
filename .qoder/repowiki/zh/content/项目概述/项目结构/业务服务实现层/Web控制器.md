...
- [ComponentHealthVO.java](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/model/vo/ComponentHealthVO.java)
- [HealthStatusVO.java](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/model/vo/HealthStatusVO.java)
...
## 更新摘要
**变更内容**
- 新增健康检查控制器模块，提供 `/res/v1/health/custom` 端点
- 添加数据库和应用程序组件的健康检查功能
- 实现完整的错误处理和参数验证机制
- 新增集成测试覆盖所有健康检查场景
...
## 引言
本文件聚焦于Web控制器模块，特别是TestController和新增的HealthController作为系统RESTful API入口的设计与实现。文档将系统阐述HTTP接口定义、请求参数处理、响应数据封装等关键点；深入解析ApiResult统一响应模型的设计理念与使用方法；说明控制器层如何通过标准API对外暴露业务能力，并阐明与门面层的交互关系。

## 项目结构
Web控制器位于biz-service-impl模块的web子包中，采用Spring MVC注解驱动的REST风格设计。该模块同时包含：
- 控制器层：TestController、HealthController
- 响应封装：ApiResult统一响应模型
- 健康检查VO：HealthStatusVO、ComponentHealthVO
...
HC["controller/HealthController.java"]
V["model/vo/ApiResult.java"]
CHV["model/vo/ComponentHealthVO.java"]
HSV["model/vo/HealthStatusVO.java"]
U["util/WebUtil.java"]
...
CHV2["health/ComponentHealthVO.java"]
HSV2["health/HealthStatusVO.java"]
end
C --> V
HC --> V
HC --> HSV2
...
- [HealthController.java:25-43](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L25-L43)
...
- [HealthStatusVO.java:1-39](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/model/vo/HealthStatusVO.java#L1-L39)
- [ComponentHealthVO.java:1-33](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/model/vo/ComponentHealthVO.java#L1-L33)
...
- HealthController：健康检查控制器，提供业务级健康检查API，支持数据库和应用程序组件的健康状态查询。
...
**章节来源**
- [TestController.java:48-50](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/TestController.java#L48-L50)
- [HealthController.java:16-26](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L16-L26)
...
Controller->>Service : "调用健康检查服务"
Service-->>Controller : "健康状态结果"
...
- 路由前缀：/res/v1/health
- 端点：/custom（GET）
- 功能：提供业务级健康检查API，支持数据库和应用程序组件的健康状态查询
- 参数验证：支持component参数（database、application），未知组件返回错误
- 错误处理：完整的异常捕获和日志记录
- 性能监控：记录请求处理时间和健康状态

```mermaid
flowchart TD
Start(["健康检查请求"]) --> Param{"验证参数"}
Param --> |有效| Call["调用ICustomHealthService.getHealthStatus()"]
Param --> |无效| Error["返回ApiResult.fail()"]
Call --> Success["封装ApiResult.success()"]
Error --> End(["结束"])
Success --> End
```

**图表来源**
- [HealthController.java:36-65](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L36-L65)

**章节来源**
- [HealthController.java:16-66](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L16-L66)
...
### 健康检查VO模型
- HealthStatusVO：健康状态响应值对象，包含整体状态、时间戳和组件详情
- ComponentHealthVO：组件健康状态值对象，包含组件状态和详细信息
- 注意：Web控制器中的VO类已标记为@Deprecated，实际使用应参考common-util中的类

**章节来源**
- [HealthStatusVO.java:11-38](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/model/vo/HealthStatusVO.java#L11-L38)
- [ComponentHealthVO.java:10-32](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/model/vo/ComponentHealthVO.java#L10-L32)
...
```mermaid
graph LR
Controller["TestController/HealthController"] --> IFace["门面接口<br/>PayChannelService/NotifyService/ICustomHealthService"]
IFace --> Impl["门面实现"]
```
...
- [ICustomHealthService.java:11-19](file://core-service/src/main/java/com/magicliang/transaction/sys/core/service/ICustomHealthService.java#L11-L19)
...
```mermaid
graph TB
Controller["TestController/HealthController"] --> Util["WebUtil"]
...
**图表来源**
- [TestController.java:48-50](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/TestController.java#L48-L50)
- [HealthController.java:33-34](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L33-L34)
- [WebUtil.java:34-512](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/util/WebUtil.java#L34-L512)
- [RestExceptionHandler.java:24-39](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/advice/RestExceptionHandler.java#L24-L39)
- [WebMvcConfig.java:25-75](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/config/WebMvcConfig.java#L25-L75)
- [InnerApiAuthenticator.java:20-26](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/interceptor/InnerApiAuthenticator.java#L20-L26)
...
- 健康检查：验证component参数的有效性，检查服务层实现的可用性
...
- [HealthController.java:48-52](file://biz-service-impl/src/main/java/com/magicliang/transaction/sys/biz/service/impl/web/controller/HealthController.java#L48-L52)

## 结论
TestController和新增的HealthController共同构成了Web控制器的核心入口，通过清晰的REST接口设计、完善的请求参数处理、统一的ApiResult响应封装，以及与门面层的解耦交互，实现了标准的API对外暴露。结合WebUtil、RestExceptionHandler、WebMvcConfig与拦截器，构建了健壮、可扩展且易于维护的Web层架构。新增的健康检查控制器为系统的可观测性和运维监控提供了重要支撑，支持数据库和应用程序组件的实时健康状态查询。