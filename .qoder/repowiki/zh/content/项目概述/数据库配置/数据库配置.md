...
- [04-mariadb-deployment.yaml（staging）](file://deploy/k8s/staging/04-mariadb-deployment.yaml)
- [03-mariadb-pvc.yaml（staging）](file://deploy/k8s/staging/03-mariadb-pvc.yaml)
- [05-mariadb-service.yaml（staging）](file://deploy/k8s/staging/05-mariadb-service.yaml)
- [01-mariadb-secret.yaml（staging）](file://deploy/k8s/staging/01-mariadb-secret.yaml)
- [01-mariadb-secret.yaml（prod）](file://deploy/k8s/prod/01-mariadb-secret.yaml)
</cite>
...
- 部署与运维：Kubernetes YAML（staging/prod 环境的 MariaDB 部署、Service、PVC、Secret）
...
DEP["04-mariadb-deployment.yamlstaging"]
SVC["05-mariadb-service.yamlstaging"]
PVC["03-mariadb-pvc.yamlstaging"]
SEC["01-mariadb-secret.yamlstaging/prod"]
end
YML --> TC
YML --> M4J
YML --> DS
...
- [04-mariadb-deployment.yaml（staging）:1-56](file://deploy/k8s/staging/04-mariadb-deployment.yaml#L1-L56)
- [05-mariadb-service.yaml（staging）:1-17](file://deploy/k8s/staging/05-mariadb-service.yaml#L1-L17)
- [03-mariadb-pvc.yaml（staging）:1-15](file://deploy/k8s/staging/03-mariadb-pvc.yaml#L1-L15)
- [01-mariadb-secret.yaml（staging）:1-13](file://deploy/k8s/staging/01-mariadb-secret.yaml#L1-L13)
- [01-mariadb-secret.yaml（prod）:1-14](file://deploy/k8s/prod/01-mariadb-secret.yaml#L1-L14)
...
- mariadb4j（local-mariadb4j-dev）
  ...
  - 架构不兼容
    - ARM 芯片请改用 local-tc-dev 或 local-mysql-dev
  - 初始化异常
...
- [DataSourceConfig.java:33-52](file://common-dal/src/main/java/com/magicliang/transaction/sys/common/dal/datasource/DataSourceConfig.java#L33-L52)
- [04-mariadb-deployment.yaml（staging）:45-49](file://deploy/k8s/staging/04-mariadb-deployment.yaml#L45-L49)
- [05-mariadb-service.yaml（staging）:10-16](file://deploy/k8s/staging/05-mariadb-service.yaml#L10-L16)
- [03-mariadb-pvc.yaml（staging）:10-15](file://deploy/k8s/staging/03-mariadb-pvc.yaml#L10-L15)
- [01-mariadb-secret.yaml（staging）:10-12](file://deploy/k8s/staging/01-mariadb-secret.yaml#L10-L12)
- [01-mariadb-secret.yaml（prod）:10-13](file://deploy/k8s/prod/01-mariadb-secret.yaml#L10-L13)

## 结论
- local-tc-dev（推荐）：通过 Testcontainers 自动管理 MariaDB 容器，支持 Docker/Podman，一键创建双数据库并执行初始化脚本，适合开发与测试全流程自动化
- local-mariadb4j-dev：无需 Docker 的内嵌 MariaDB，适合快速本地验证，但受限于 x86_64 架构
