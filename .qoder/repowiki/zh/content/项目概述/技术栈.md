...
## 引言
本技术栈文档面向领域驱动交易系统，系统采用多模块 Gradle 构建，后端技术栈以 Java 8、Spring Boot 2.7.18、Spring Cloud 2021.0.8 为核心，结合 MyBatis 2.3.2 与 Spring Data JPA 提供持久层能力，使用 Log4j2 作为日志框架，并集成了 OpenTelemetry 2.10.0 以实现可观测性。测试框架采用 JUnit 5.9.3，构建工具使用 Gradle 8.6，整体设计强调模块化、可维护性和可扩展性。

## 项目结构
项目采用多模块结构，通过 Gradle 的 settings.gradle 统一管理子模块，根构建脚本负责全局插件、依赖管理和仓库配置，子模块各自定义功能边界与依赖范围。

```mermaid
graph TB
Root["根项目<br/>build.gradle"] --> Settings["模块清单<br/>settings.gradle"]
Root --> SubBiz["biz-service-impl<br/>业务实现"]
Root --> SubShared["biz-shared<br/>共享接口"]
Root --> SubDAL["common-dal<br/>数据访问层"]
Root --> SubServiceFacade["common-service-facade<br/>服务门面"]
Root --> SubServiceIntegration["common-service-integration<br/>集成服务"]
Root --> SubUtil["common-util<br/>通用工具"]
Root --> SubCoreModel["core-model<br/>核心模型"]
Root --> SubCoreService["core-service<br/>核心服务"]
Settings --> SubBiz
Settings --> SubShared
Settings --> SubDAL
...
- 构建工具
  - Gradle 8.6：提供并行构建、配置按需执行、构建缓存等性能优化，配合工具链与 Java 8 兼容。
...
## 架构概览
系统采用分层与模块化设计，业务实现模块对外提供 Web 服务，共享模块定义接口契约，DAL 模块封装数据访问，核心模型与服务模块承载领域逻辑与策略。
...
subgraph "业务层"
Facade["门面层<br/>biz-shared 接口"]
Impl["实现层<br/>biz-service-impl"]
end
subgraph "核心领域"
Model["核心模型<br/>core-model"]
Service["核心服务<br/>core-service"]
end
subgraph "数据访问"
DAL["数据访问层<br/>common-dal"]
JPA["Spring Data JPA"]
...
```mermaid
graph LR
BizImpl["biz-service-impl"] --> Shared["biz-shared"]
BizImpl --> Facade["common-service-facade"]
BizImpl --> DAL["common-dal"]
DAL --> Util["common-util"]
DAL --> JPA["Spring Data JPA"]
DAL --> MyBatis["MyBatis 2.3.2"]
DAL --> DB["MySQL/MariaDB"]
CoreSvc["core-service"] --> CoreModel["core-model"]
...
## 结论
本技术栈以 Java 8 为基础，结合 Spring Boot 2.7.18 与 Spring Cloud 2021.0.8 构建稳定的企业级后端服务；通过 JPA 与 MyBatis 双轨持久层满足不同场景需求；JUnit 5.9.3 保障测试质量；Log4j2 提供灵活的日志输出；OpenTelemetry 2.10.0 实现可观测性；Gradle 8.6 提升构建效率与一致性。模块化设计使系统具备良好的可维护性与扩展性，适合持续演进的领域驱动交易系统。