...
public class TransPayOrderValidator {
    ...
    public static void validateBeforeInsert(TransPayOrderEntity payOrder) {
        ...
        AssertUtils.assertNotNull(TransSysConfigEnum.getByCode(payOrder.getSysCode()), INVALID_PAY_ORDER_ERROR, "invalid sysCode：" + payOrder);

        AssertUtils.assertNotBlank(payOrder.getBizIdentifyNo(), INVALID_PAY_ORDER_ERROR, "invalid bizIdentifyNo：" + payOrder);
        AssertUtils.assertNotBlank(payOrder.getBizUniqueNo(), INVALID_PAY_ORDER_ERROR, "invalid bizUniqueNo：" + payOrder);

        final Long money = payOrder.getMoney();
        AssertUtils.isTrue(null != money && money > 0L, INVALID_PAY_ORDER_ERROR, "invalid money：" + payOrder);

        AssertUtils.assertNotNull(TransFundAccountingEntryTypeEnum.getByCode(payOrder.getAccountingEntry().intValue()), INVALID_PAY_ORDER_ERROR, "invalid accountingEntry：" + payOrder);
